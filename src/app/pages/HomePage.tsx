...
/* ─── HERO CAROUSEL ─── */
function HeroCarousel({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 p-5 lg:p-7">
        ...
        <AppLink to={getArticleUrl(active.slug, tenantSlug)}>
...
/* ─── HERO GRID LAYOUT ─── */
function HeroGrid({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  if (!articles.length) return null;
  const [main, ...others] = articles.slice(0, 5);
  
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {/* Main large article */}
      <AppLink to={getArticleUrl(main.slug, tenantSlug)} className="group relative rounded-xl overflow-hidden shadow-xl md:row-span-2">
        <div className="aspect-[4/3] md:h-[480px] overflow-hidden">
          <ImageWithFallback src={thumb(main)} alt={main.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        </div>
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
        <div className="absolute bottom-0 left-0 right-0 p-5">
          <span className="inline-block bg-red-600 text-white text-[10px] font-bold uppercase px-2 py-0.5 rounded mb-2">{main.category_name}</span>
          <h2 className="text-xl lg:text-2xl font-bold text-white leading-tight line-clamp-2 hover:underline mb-2">{main.title}</h2>
          <p className="text-sm text-white/80 line-clamp-2">{main.excerpt}</p>
        </div>
      </AppLink>
      
      {/* Grid of smaller articles */}
      {others.map(article => (
        <AppLink key={article.id} to={getArticleUrl(article.slug, tenantSlug)} className="group relative rounded-xl overflow-hidden shadow-lg">
          <div className="aspect-[16/9] overflow-hidden">
            <ImageWithFallback src={thumb(article)} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          </div>
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <span className="inline-block bg-red-600 text-white text-[9px] font-bold uppercase px-2 py-0.5 rounded mb-1">{article.category_name}</span>
            <h3 className="text-sm font-bold text-white leading-tight line-clamp-2 hover:underline">{article.title}</h3>
          </div>
        </AppLink>
      ))}
    </div>
...
/* ─── HERO SINGLE LARGE ─── */
function HeroSingleLarge({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  if (!articles.length) return null;
  const main = articles[0];
  
  return (
    <AppLink to={getArticleUrl(main.slug, tenantSlug)} className="group relative rounded-xl overflow-hidden shadow-xl block">
      <div className="h-[480px] overflow-hidden">
        <ImageWithFallback src={thumb(main)} alt={main.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
      </div>
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent" />
      <div className="absolute bottom-0 left-0 right-0 p-8 max-w-4xl">
        <span className="inline-block bg-red-600 text-white text-xs font-bold uppercase px-3 py-1 rounded mb-3">{main.category_name}</span>
        <h1 className="text-3xl lg:text-4xl font-bold text-white leading-tight mb-3 hover:underline">{main.title}</h1>
        <p className="text-base text-white/90 line-clamp-2 mb-4 max-w-2xl">{main.excerpt}</p>
        <div className="flex items-center gap-4 text-sm text-white/80">
          <span className="flex items-center gap-1"><Clock className="h-4 w-4" />{getRelativeTime(main.publish_at)}</span>
          <span className="flex items-center gap-1"><User className="h-4 w-4" />{main.author_name}</span>
          <span className="flex items-center gap-1"><Eye className="h-4 w-4" />{formatViews(main.views_count)}</span>
        </div>
      </div>
    </AppLink>
  );
...
/* ─── HERO WRAPPER (selects layout based on theme settings) ─── */
function Hero({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  const { siteSettings } = useCms();
  const theme = (siteSettings?.theme_config as Record<string, unknown> | undefined) ?? {};
  const heroLayout = String(theme.hero_layout ?? 'Full Width Slider');
  
  if (heroLayout === 'Grid Layout') {
    return <HeroGrid articles={articles} tenantSlug={tenantSlug} />;
  } else if (heroLayout === 'Single Large') {
    return <HeroSingleLarge articles={articles} tenantSlug={tenantSlug} />;
  }
  
  // Default: Full Width Slider
  return <HeroCarousel articles={articles} tenantSlug={tenantSlug} />;
}

/* ─── TRENDING SIDEBAR ─── */
function TrendingSidebar({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <div className="space-y-3">
        {articles.slice(0, 5).map((a, i) => (
          <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="flex gap-3 group">
...
/* ─── MOST READ ─── */
function MostReadSidebar({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="flex items-center gap-1.5 text-sm font-bold text-gray-900">
...
function BreakingNewsGrid({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {pageArticles.map(a => (
          <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="group bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
...
function LatestNewsSection({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  const [page, setPage] = useState(0);
  const perPage = 6;
  const totalPages = Math.ceil(articles.length / perPage);
  const pageArticles = articles.slice(page * perPage, (page + 1) * perPage);
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {pageArticles.map(a => (
          <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="flex gap-3 group bg-white rounded-lg border border-gray-100 p-3 hover:shadow-md hover:-translate-y-0.5 transition-all">
...
/* ─── VIDEO NEWS ─── */
function VideoNewsSection({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug: string }) {
  if (!articles.length) return null;
  return (
    <section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {articles.slice(0, 4).map(a => (
          <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="group block">
...
function CategorySection({ title, articles, href, tenantSlug }: { title: string; articles: PublicArticle[]; href: string; tenantSlug: string }) {
  if (!articles.length) return null;
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-5">
        {/* Lead card */}
        <AppLink to={getArticleUrl(lead.slug, tenantSlug)} className="group relative rounded-xl overflow-hidden shadow-sm">
    ...
    <section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {articles.slice(0, 8).map(a => (
          <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="group relative rounded-xl overflow-hidden shadow-sm">
            <div className="relative w-full" style={{ paddingBottom: '75%' }}>
              <ImageWithFallback src={thumb(a)} alt={a.title} className="absolute inset-0 w-full h-full object-cover object-center transition-transform duration-300 group-hover:scale-110" />
    ...
    <section className="bg-gray-900 rounded-xl p-6 shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {articles.slice(0, 3).map(a => (
          <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="group bg-gray-800 rounded-lg p-4 hover:bg-gray-750 transition-colors">
...
export function HomePage() {
  ...
  // SEO Metadata
  const pageTitle = resolveSEOTitle(undefined, undefined, seoDefaults);
  const pageDescription = resolveSEODescription(undefined, undefined, seoDefaults);
  const pageImage = resolveSEOImage(undefined, seoDefaults);
  const canonicalUrl = normalizeCanonicalUrl('/', seoDefaults?.canonical_base_url);
  const robots = generateRobotsContent(seoDefaults);
  
  // Structured Data
  const structuredData = [];
  
  if (seoDefaults?.show_publication_schema && seoDefaults?.canonical_base_url) {
    structuredData.push(generateWebSiteStructuredData({
      name: seoDefaults.site_title || tenant?.name || 'News Site',
      url: seoDefaults.canonical_base_url,
      description: seoDefaults.site_description || undefined,
      potentialAction: {
        searchUrl: `${seoDefaults.canonical_base_url}/search?q={search_term_string}`,
      },
    }));
    ...
    structuredData.push(generateOrganizationStructuredData({
      name: seoDefaults.site_title || tenant?.name || 'News Site',
      url: seoDefaults.canonical_base_url,
      logo: seoDefaults.default_image_url || undefined,
      description: seoDefaults.site_description || undefined,
    }));
  }
  ...
  return (
    <div className="bg-[#f8f9fa]">
      ...
      <PageMetadata
        ...
        robots={robots}
        ogType="website"
        ogSiteName={seoDefaults?.og_site_name || seoDefaults?.site_title || undefined}
        ogTitle={seoDefaults?.og_title || pageTitle}
        ogDescription={seoDefaults?.og_description || pageDescription}
        ogImage={seoDefaults?.og_image || pageImage}
        ogImageWidth={seoDefaults?.og_image_width || undefined}
        ogImageHeight={seoDefaults?.og_image_height || undefined}
        twitterCard={seoDefaults?.twitter_card || 'summary_large_image'}
        twitterSite={seoDefaults?.twitter_site || undefined}
        twitterCreator={seoDefaults?.twitter_creator || undefined}
        twitterTitle={seoDefaults?.twitter_title || pageTitle}
        twitterDescription={seoDefaults?.twitter_description || pageDescription}
        twitterImage={seoDefaults?.twitter_image || pageImage}
        language={seoDefaults?.default_language || 'en'}
        locale={seoDefaults?.default_locale || 'en_US'}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-4">
              ...
              <div className="hidden lg:flex flex-col gap-4">
                <TrendingSidebar articles={trendingArticles} tenantSlug={tenantSlug} />
              </div>
            </div>

            {/* Featured Story Grid */}
            <FeaturedStoryGrid articles={featuredArticles.length >= 3 ? featuredArticles : articles.slice(0, 6)} tenantSlug={tenantSlug} />

            {/* Breaking News Grid */}
            <BreakingNewsGrid articles={breakingArticles.length >= 3 ? breakingArticles : articles.slice(0, 6)} tenantSlug={tenantSlug} />

            {/* Mid-content Ad */}
            <SmartAd placement="homepage_mid_banner" />

            {/* Latest News */}
            <LatestNewsSection articles={latestArticles} tenantSlug={tenantSlug} />

            {/* Live TV Banner */}
            <LiveTVBanner />

            {/* Video News */}
            <VideoNewsSection articles={videoArticles} tenantSlug={tenantSlug} />

            {/* Bihar News */}
            <CategorySection title="Bihar News (बिहार)" articles={biharNews} href="/category/बिहार" tenantSlug={tenantSlug} />

            {/* National News */}
            <CategorySection title="National News" articles={nationalNews} href="/search?q=national" tenantSlug={tenantSlug} />
