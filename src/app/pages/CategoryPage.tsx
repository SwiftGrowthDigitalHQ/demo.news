...
import { PageMetadata, resolveSEOTitle, resolveSEODescription, resolveSEOImage, normalizeCanonicalUrl, generateRobotsContent, generateBreadcrumbStructuredData } from '../lib/seoMetadata';
...
function MostReadWidget({ articles, tenantSlug }: { articles: PublicArticle[]; tenantSlug?: string | null }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-gray-900 flex items-center gap-1.5">
      ...
      <div className="space-y-3">
        {articles.slice(0, 5).map((a, i) => (
          <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="flex gap-2 group">
...
export function CategoryPage({ slug }: { slug: string }) {
  ...
  // SEO Metadata - use category-specific SEO fields, fallback to SEO Manager defaults
  const categoryName = category?.name || slug;
  const pageTitle = resolveSEOTitle(
    `${categoryName} - Latest News | ${tenant?.name || 'News'}`,
    category?.seo_title || undefined,
    seoDefaults
  );
  const pageDescription = resolveSEODescription(
    category?.description || `Latest ${categoryName} news, updates, and stories`,
    category?.seo_description || undefined,
    seoDefaults
  );
  const pageImage = resolveSEOImage(
    featuredArticle?.featured_image || undefined,
    seoDefaults
  );
  const canonicalUrl = normalizeCanonicalUrl(`/category/${activeSlug}`, seoDefaults?.canonical_base_url);
  
  // Respect category indexing settings
  let categoryRobots = generateRobotsContent(seoDefaults);
  if (seoDefaults?.category_indexing === false) {
    categoryRobots = 'noindex,follow';
  }
  ...
  if (seoDefaults?.show_breadcrumb_schema && seoDefaults?.canonical_base_url) {
    structuredData.push(generateBreadcrumbStructuredData([
      ...
      { name: categoryName, url: `${seoDefaults.canonical_base_url}/category/${activeSlug}` },
    ]));
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <PageMetadata
        ...
        ogTitle={category?.seo_title || pageTitle}
        ogDescription={category?.seo_description || pageDescription}
        ogImage={pageImage}
        ...
        twitterTitle={category?.seo_title || pageTitle}
        twitterDescription={category?.seo_description || pageDescription}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-4">
                ...
                {featuredArticle && (
                  <div>
                    ...
                    <AppLink to={getArticleUrl(featuredArticle.slug, tenantSlug)} className="group block rounded-xl overflow-hidden bg-white border border-gray-100 hover:shadow-md transition-shadow">
                ...
                {topHeadlines.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-3 bg-white rounded-xl border border-gray-100 p-3">
                      {topHeadlines.map(a => (
                        <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="flex gap-3 group border-b border-gray-50 last:border-0 pb-3 last:pb-0">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                  {trendingArticles.map(a => (
                    <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition-all">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {videoArticles.slice(0, 2).map(a => (
                      <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="flex gap-3 group bg-white rounded-lg border border-gray-100 p-2 hover:shadow-sm transition-shadow">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {photoArticles.slice(0, 4).map(a => (
                      <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="group relative rounded-lg overflow-hidden aspect-[4/3]">
                        <ImageWithFallback src={thumb(a)} alt={a.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
