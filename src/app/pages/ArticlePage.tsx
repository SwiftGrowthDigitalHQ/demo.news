...
import { CalendarDays, Eye, MapPin, User, Facebook, MessageCircle, Share2, Copy, Send } from 'lucide-react';
...
import { PageMetadata, resolveSEOTitle, resolveSEODescription, resolveSEOImage, normalizeCanonicalUrl, generateRobotsContent, generateArticleStructuredData, generateBreadcrumbStructuredData } from '../lib/seoMetadata';
...
export function ArticlePage({ slug }: { slug: string }) {
  ...
  // Track article view - with session-based deduplication to prevent double-counting
  useEffect(() => {
    if (!article) return;
    
    // Session-based deduplication: only track once per article per session
    const viewTrackingKey = `article_view_${article.id}`;
    const lastTrackedTime = sessionStorage.getItem(viewTrackingKey);
    const now = Date.now();
    
    // Only track if:
    // 1. Never tracked in this session, OR
    // 2. Last tracked more than 30 seconds ago (allows legitimate refresh as new view)
    if (!lastTrackedTime || (now - parseInt(lastTrackedTime, 10)) > 30000) {
      sessionStorage.setItem(viewTrackingKey, String(now));
      void trackAnalyticsEvent({ 
        event_type: 'page_view', 
        page_path: `/article/${article.slug}`, 
        article_id: article.id, 
        category_id: article.category_id, 
        metadata: { page: 'article' } 
      }).catch(() => {});
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [article?.id]); // Only track when article ID changes, not when article object re-renders
  ...
  // SEO Metadata - use article-specific SEO fields, fallback to SEO Manager defaults
  const pageTitle = resolveSEOTitle(
    `${article.title} | ${tenant?.name || 'News'}`,
    article.seo_title || undefined,
    seoDefaults
  );
  const pageDescription = resolveSEODescription(
    article.excerpt || undefined,
    article.seo_description || undefined,
    seoDefaults
  );
  const pageImage = resolveSEOImage(
    getArticleThumbnail(article.featured_image, article.video_url),
    seoDefaults
  );
  const canonicalUrl = normalizeCanonicalUrl(`/article/${article.slug}`, seoDefaults?.canonical_base_url);

  // Robots precedence: global robots setting (index/noindex, follow/nofollow).
  // category_indexing controls CATEGORY pages only, not individual articles.
  const articleRobots = generateRobotsContent(seoDefaults);
  
  // Structured Data
  const structuredData = [];
  
  // Article Schema
  if (seoDefaults?.show_article_schema && seoDefaults?.canonical_base_url) {
    structuredData.push(generateArticleStructuredData({
      title: article.title,
      description: article.excerpt || undefined,
      image: pageImage,
      // PublicArticle only exposes publish_at; no updated_at on the public type
      publishedTime: article.publish_at || new Date().toISOString(),
      modifiedTime: article.publish_at || undefined,
      author: article.author_name || seoDefaults.default_author || undefined,
      section: article.category_name,
      url: `${seoDefaults.canonical_base_url}/article/${article.slug}`,
      publisher: {
  ...
  // Breadcrumb Schema
  if (seoDefaults?.show_breadcrumb_schema && seoDefaults?.canonical_base_url) {
    structuredData.push(generateBreadcrumbStructuredData([
      { name: 'Home', url: seoDefaults.canonical_base_url },
      { name: article.category_name, url: `${seoDefaults.canonical_base_url}/category/${article.category_slug}` },
      { name: article.title, url: `${seoDefaults.canonical_base_url}/article/${article.slug}` },
    ]));
  }
  ...
  return (
    <div className="bg-[#f8f9fa]">
      ...
      <PageMetadata
        ...
        ogDescription={article.seo_description || article.excerpt || undefined}
        ogImage={pageImage}
        ...
        twitterTitle={article.seo_title || article.title}
        twitterDescription={article.seo_description || article.excerpt || undefined}
        twitterImage={pageImage}
        articlePublishedTime={article.publish_at || undefined}
        articleModifiedTime={article.publish_at || undefined}
            ...
            <section className="mb-4">
              ...
              {related.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {related.map(a => (
                    <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all">
            ...
            <div className="sticky top-[72px] space-y-4 pb-6">
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <div className="space-y-3">
                  {trending.map((a, i) => (
                    <AppLink key={a.id} to={getArticleUrl(a.slug, tenantSlug)} className="flex gap-2.5 group">
