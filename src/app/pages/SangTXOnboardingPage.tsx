...
import { RESERVED_PLATFORM_SLUGS, isReservedSlug, clearTenantCache } from '../lib/tenantRegistry';
...
function validate(step: number, data: OnboardingData): Partial<Record<string, string>> {
  ...
  if (step === 1) {
    ...
    else if (isReservedSlug(data.slug)) errors.slug = 'This slug is reserved. Please choose another.';
...
export function SangTXOnboardingPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Guard against localhost being baked into bundle via VITE_SITE_URL

      const rawSiteUrl2 = import.meta.env.VITE_SITE_URL as string | undefined;

      const siteUrl = (rawSiteUrl2 && !rawSiteUrl2.includes('localhost'))
