...
export function SuperAdminDashboard() {
  ...
  async function loadData() {
    ...
    if ('error' in metricsData) {
      console.error('Metrics error:', metricsData.message);
      setMetrics(null);
    } else {
      setMetrics(metricsData);
    }
    
    if (allTenants.data && allTenants.data.length > 0) {
      const now = new Date();
      const sevenDaysFromNow = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
      
      const expiringTenants = allTenants.data.filter(t => {
        const endsAt = t.subscription_ends_at ? new Date(t.subscription_ends_at) : null;
        return endsAt && endsAt >= now && endsAt <= sevenDaysFromNow;
      });
      
      const overdueTenants = allTenants.data.filter(t => {
        const endsAt = t.subscription_ends_at ? new Date(t.subscription_ends_at) : null;
