...
export function Header() {
  ...
  const { siteSettings, breakingNews, tenantSlug } = useCms();
  ...
  // Split brand name: last word gets red color (e.g. "My News" → "My " + "News")
    ...
    <header className="w-full z-50">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        <div className="mx-auto max-w-[1400px] px-4">
          <div className="hidden lg:flex items-center h-11 gap-0 overflow-x-auto scrollbar-hide">
            {NAV_ITEMS.map(item => {
              // Use tenant slug for home link
              const itemPath = item.path === '/' && tenantSlug ? `/${tenantSlug}` : item.path;
              const isActive = pathname === itemPath || (itemPath !== '/' && !itemPath.endsWith(tenantSlug || '') && pathname.startsWith(itemPath));
              return (
                <AppLink
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[200] lg:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-[85vw] max-w-[320px] bg-white shadow-2xl overflow-y-auto animate-slide-in-left">
            ...
            <nav className="p-4 space-y-0.5">
              {NAV_ITEMS.map(item => {
                ...
                const itemPath = item.path === '/' && tenantSlug ? `/${tenantSlug}` : item.path;
                return (
                  <AppLink key={item.path + item.label} to={itemPath}
