...
export function BreakingNewsSlider() {
  const { breakingNews, articles, siteSettings, tenantSlug } = useCms();
  ...
  const breakingHighlights = useMemo(
    () => breakingNews.slice(0, 6).map(item => {
      ...
      return {
        ...
        href: item.link_url || (article ? getArticleUrl(article.slug, tenantSlug) : `/search?q=${encodeURIComponent(item.headline)}`),
      };
    }),
    [articles, breakingNews],
  );
  ...
  return (
    <section className="w-full rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="grid gap-0 grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.15fr_0.75fr_0.8fr]">
        ...
        <div className="hidden lg:block border-t border-gray-200 bg-white p-5 lg:border-t-0 lg:border-l">
          <div className="mb-4 flex items-center justify-between">
